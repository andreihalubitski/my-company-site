...
jQuery.fn.myig = function(g, h, j) {
    var k = (jQuery(this).attr("id") != null || jQuery(this).attr("id") != undefined ? '#' + jQuery(this).attr("id") : '.' + jQuery(this).attr("class"));
    myig_gallery(k, "");
    ...
    function myig_gallery(e, f) {
        jQuery.ajax({
            ...
            var d = '';
            jQuery.each(c.data, function(i, a) {
                var b = '';
                ...
                d += '<li class="user_gallery">';
                d += '	<a href="' + c.data[i].images.standard_resolution.url.replace(/\\/, "") + '" class="instagram_link" title="' + b + '">';
                d += '		<img src="' + c.data[i].images.thumbnail.url.replace(/\\/, "") + '" alt="" title="' + b + '">';
                d += '	</a>';
                d += '</li>'
            });
            jQuery('.instagram-feed').append(d);
			jQuery('.instagram_link').magnificPopup({type:'image'});
        });
    }
}

jQuery(document).ready(function(){
	if (jQuery('.instagram-feed').size() > 0) {
		jQuery(".instagram-feed").myig(
			ins_id = jQuery(".instagram-feed").attr('data-id'), // your user_id
			ins_count = jQuery(".instagram-feed").attr('data-count'), // Count of media to return
			ins_token = jQuery(".instagram-feed").attr('data-token') // your token
		);			
	}
});